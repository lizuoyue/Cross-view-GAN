...
def define_S(input_nc, output_nc, ngf, net_type, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
...
def define_G(input_nc, output_nc, ngf, net_type, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
...
def define_D(input_nc, ndf, net_type, n_layers_D=3, norm='batch', use_sigmoid=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)

    if net_type == 'basic':
        net = NLayerDiscriminator(input_nc, ndf, n_layers=3, norm_layer=norm_layer, use_sigmoid=use_sigmoid)
    elif net_type == 'n_layers':
...
def define_X(input_nc, output_nc, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = XNet(input_nc, output_nc)
...
class GANLoss(nn.Module):
    ...
    def __call__(self, pred, target_is_real, weight=None):
        ...
        if weight is None:
            loss_final = loss_raw#torch.mean(loss_raw)
        else:

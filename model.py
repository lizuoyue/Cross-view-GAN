...
class L2RAllModel:
    ...
    def backward_D(self):
        ...
        for i in range(self.num_classes):
            mask = self.g_masks[i]
            mask_3 = torch.cat([mask, mask, mask], 1)
            mask_low_res = None # (self.toLowResMask(mask) > 0).float()
    ...
    def backward_G(self):
        ...
        for i in range(self.num_classes):
            ...
            loss_G_GAN = self.criterionGAN(pred_fake, True, None)
...
class L2RNoiseModel:
    ...
    def set_input(self, input):
        ...
        self.g_masks = [(input['street_label'] == i).float().to(self.device) for i in range(self.num_classes)]
        self.g_noise = input['noise'].to(self.device)
